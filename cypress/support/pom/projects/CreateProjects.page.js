export class CreateProjectsPage {
  ...
  txtProjectName = () =>
    cy.get('input[class="form-control ng-untouched ng-pristine ng-valid"]');

  btnEmailInput = () =>
    cy.get(
      '[class="select2-selection select2-selection--multiple"][title="email recipents"]'
    );
  sltEmailOptions = () => cy.get('[class="select2-results__option"]');
  txtDescriptionInput = () => cy.get('[class="note-editing-area"]');

  toastProjectCreated = () =>
    cy.get('[id="toasty"]').contains("Project created.");

  btnSave = () => cy.get('[title="Save"]');

  createNewProject(projectName, timeStamp, projectDescription) {
    ...
    this.txtDescriptionInput().type(projectDescription);
